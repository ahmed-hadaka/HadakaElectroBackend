...
import com.hadaka_electro.internal.customer.CustomerDTO;
import com.hadaka_electro.internal.customer.CustomerRepository;
...
public class CheckObjectUniqueness {
    ...
    public CheckObjectUniqueness(BrandRepository brandRepository, ProductRepository productRepository, UserRepository userRepository, CategoryRepository categoryRepository, CustomerRepository customerRepository) {
    ...
    public void checkUserEmailUniqueness(JoinPoint joinPoint) throws DuplicatedObjectException {
        Object[] args = joinPoint.getArgs();
    ...
    @Before("execution(* com.hadaka_electro.internal.customer.CustomerController.updateCustomer(..))")
    public void checkCustomerEmailUniqueness(JoinPoint joinPoint) throws DuplicatedObjectException {

        CustomerDTO customerDTO = (CustomerDTO) joinPoint.getArgs()[0];

        long dtoId = customerDTO.getId();
        String email = customerDTO.getEmail();

        Optional<Customer> customer = customerRepository.findByEmail(email);
        if (customer.isPresent()) {
            if (customer.get().getId() != dtoId)
                throw new DuplicatedObjectException("The email " + email + " is already taken!");
        }
    }
}
